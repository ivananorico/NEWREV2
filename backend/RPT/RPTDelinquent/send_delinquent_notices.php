<?php
// backend/RPT/RPTDelinquent/send_delinquent_notices.php
...
$smtp_config = [
    ...
    'from_name' => 'Caloocan City Treasury Department'
];

// Function to log email activity to file
function logEmailToFile($data) {
    $logDir = dirname(__DIR__, 3) . '/logs/email_logs';
...
function getEmailTemplate($type, $data, $customMessage = '') {
    ...
    $treasury_contact = "treasury@caloocan.gov.ph";
    $treasury_phone = "(02) 1234-5678";
    $treasury_address = "2nd Floor, City Hall Building, Caloocan City";
    ...
    $propertyLocation = $data['lot_location'] ?? 'Not specified';
    $tdn = $data['tdn'] ?? $data['land_tdn'] ?? 'N/A';
    ...
    $templates = [
        'standard' => [
            'subject' => "Notice of Delinquent Real Property Tax - $tdn",
            'body' => "
                <!DOCTYPE html>
                <html>
                <head>
                ...
                <body>
                    <div class='container'>
                        <div class='header'>
                            <h2>Notice of Delinquent Real Property Tax</h2>
                        </div>
                        <div class='content'>
                            <p>Dear <strong>$ownerName</strong>,</p>
                            
                            <p>This is to inform you that your Real Property Tax for the property below is currently delinquent:</p>
                            
                            <div class='details'>
                                <h3>Property Details:</h3>
                                <table>
                                    <tr><td><strong>TDN:</strong></td><td>$tdn</td></tr>
                                    <tr><td><strong>Location:</strong></td><td>$propertyLocation</td></tr>
                            ...
                            <p>Please settle your account immediately to avoid further penalties. You may pay at the City Treasurer's Office or through our online payment portal.</p>
                            
                            <p>Thank you for your prompt attention to this matter.</p>
                        </div>
                        <div class='footer'>
                            <p><strong>$lgu_name - Treasury Department</strong><br>
                            $treasury_address<br>
        ...
        'urgent' => [
            'subject' => "URGENT: Delinquent Real Property Tax Notice - $tdn",
            'body' => "
                <!DOCTYPE html>
                <html>
                <head>
                ...
                <body>
                    <div class='container'>
                        <div class='header'>
                            <h2>⚠️ URGENT: Delinquent Tax Notice ⚠️</h2>
                        </div>
                        <div class='content'>
                            <p>Dear <strong>$ownerName</strong>,</p>
                            
                            <div class='warning'>
                                <strong>This is an URGENT NOTICE regarding your delinquent Real Property Tax.</strong>
                                <p>Your account is now <strong>$daysLate days overdue</strong>.</p>
                            </div>
                            ...
                            <div class='details'>
                                ...
                                <table>
                                    ...
                                    <tr><td>Accrued Penalty (2% monthly):</td><td>₱$penaltyAmount</td></tr>
                                    <tr><td class='amount-due'>TOTAL AMOUNT DUE:</td><td class='amount-due'>₱$amount</td></tr>
                                </table>
                            </div>
                            ...
                            <ul>
                                ...
                                <li>Issuance of warrant of distraint and/or levy</li>
                                <li>Possible auction of property for tax delinquency</li>
                            </ul>
                            
                            " . ($customMessage ? "<p><strong>Additional Instructions:</strong><br>$customMessage</p>" : "") . "
                            
                            <p>Please settle your account within 5 days to avoid further escalation.</p>
                        </div>
                        <div class='footer'>
                            <p><strong>$lgu_name - Treasury Department</strong><br>
                            $treasury_address<br>
        ...
        'final' => [
            'subject' => "FINAL NOTICE: Delinquent Real Property Tax - Legal Action Pending - $tdn",
            'body' => "
                <!DOCTYPE html>
                <html>
                <head>
                    ...
                    <div class='container'>
                        ...
                        <div class='content'>
                            ...
                            <div class='legal-warning'>
                                ...
                                <p>Your account is now <strong>$daysLate days overdue</strong>. If payment is not received within 10 days, legal action will proceed.</p>
                            </div>
                            ...
                            <p><strong>If payment is not received within 10 days, we will be compelled to:</strong></p>
                            <ul>
                                <li>File legal actions for collection</li>
                                <li>Initiate proceedings for the levy and sale of the property</li>
                                <li>Report the delinquency to credit bureaus</li>
                                <li>Issue a warrant of distraint and/or levy</li>
                            </ul>
                            
                            " . ($customMessage ? "<p><strong>Special Instructions:</strong><br>$customMessage</p>" : "") . "
                            
                            <p>To avoid legal action, please settle your account immediately at the City Treasurer's Office.</p>
                        </div>
                        <div class='footer'>
                            <p><strong>$lgu_name - Treasury Department</strong><br>
                            $treasury_address<br>
...
foreach ($delinquents as $delinquent) {
    ...
    if (empty($delinquent['email'])) {
        $failed_count++;
        $errors[] = "No email for {$delinquent['owner_name']}";
        
        // Log to file
        logEmailToFile([
            'status' => 'failed',
            'reason' => 'no_email',
            'owner' => $delinquent['owner_name'] ?? 'Unknown',
            'tdn' => $delinquent['tdn'] ?? $delinquent['land_tdn'] ?? 'N/A',
            'timestamp' => date('Y-m-d H:i:s')
        ]);
        continue;
    }
    ...
    try {
        ...
        $mail->addAddress($delinquent['email'], $delinquent['owner_name'] ?? 'Property Owner');
        $mail->addReplyTo($smtp_config['from_email'], 'Treasury Department');
